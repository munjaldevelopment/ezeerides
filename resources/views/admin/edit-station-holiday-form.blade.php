 ...
 function getStations(city_id)
{
	if(city_id != "")
	{
		...
		$.ajax({
			type:"GET",
			url:"{{ backpack_url('getHolidayStation') }}/"+city_id,
...
function showStations(city_id, station_id)
	{
		if(city_id != "")
		{
			$("#station_id").html('<option value="">--Select--</option>');
				...
				success:function(res){ 
					if(res){
						...
						$.each(res,function(key,value){
							if(key == station_id)
							{
								$("#station_id").append('<option value="'+key+'" selected="selected">'+value+'</option>');
							}
							else
							{
								$("#station_id").append('<option value="'+key+'">'+value+'</option>');
	...
	showStations('{{ $entry->city_id }}', '{{ $entry->station_id }}');
